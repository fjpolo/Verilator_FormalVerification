...
module sfifo #(
		...
		output	reg		o_full,		// The FIFO is full
		// Read interface
		input	wire		i_rd,
...
`ifdef	VERIFIC
	...
	reg	 [LGFLEN:0]		f_distance_to_first, f_distance_to_next;
	...
	begin
		...
		f_distance_to_first = f_const_addr - rdaddr;
		if (f_distance_to_first < f_fill)
			f_addr_valid = 1'b1;
	...
	begin
		...
		f_distance_to_next = f_const_next_addr - rdaddr;
		if (f_distance_to_next < f_fill)
			f_next_valid = 1'b1;
/*
...
*/
	...
	wire f_first_set = ((w_wr)&&(wraddr==f_const_addr)
	...
	sequence	SETSEQUENCE;
		// Given the above logical wires, describe the sequence that
		// needs to take place to set two values in succession
		// within the FIFO
	endsequence
	...
	sequence	READSEQUENCE;
		// Given the wires above, what sequence would describe two
		// values being read from our FIFO in sequence?
	endsequence

	assert property (@(posedge i_clk)
		disable iff (i_reset)
		// Using these two sequences, can we write a property describing
		// what must happen?  How if we set two arbitrary values, we
		// can read those same values back later?
		//
		// Your logic here
		);

	cover property (@(posedge i_clk)
		// Just to make sure we did this right, let's make certain what
		// we just described above is even possible.  To do that,
		// describe a sequence that we can cover for that purpose
		//
		// Your logic here
		);
