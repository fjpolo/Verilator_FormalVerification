...
`ifdef FORMAL
	...
	//  1. If (i_in) is forever zero, sreg will never be completely zero
	always @(posedge i_clk) begin
		if((f_past_valid)&&(!i_reset)&&(!i_in)&&(!$past(i_in)))
			assert(sreg >= 0);
	end
	//  2. If (sreg) is ever zero sreg will remain zero until the clock after 
	// i_in is true
	always @(posedge i_clk) begin
		if((f_past_valid)&&(!i_reset)&&($past(i_in))&&(!$past(sreg)))
			assert(sreg >= 0);
	end
	//  3. if (i_in) is forever zero, prove this sequence repeats every 127 bits
	// always @(posedge i_clk) begin
	// 	if((f_past_valid)&&(!i_reset))
	// 		assert(sreg == $past(sreg, 128));
	// end
	//  4. if (i_in) is forever zero, prove this sequence will produce exactly 
